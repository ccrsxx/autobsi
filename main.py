...
class Base:
    ...
    def __init__(self, day, session, verbose, mode):
        ...
        self.sch, self.username, self.password = mode('sch'), mode('username'), mode('password')
        self.name_class = self.sch[day]['name'][session] if session is not None else self.sch[day]['name']
        self.link_class = self.sch[day]['link'][session] if session is not None else self.sch[day]['link']
...
def job(day, session=None, verbose=False, mode=get_from_config, mail=False):
    timer = time.time()
    ...
    send_mail(f'Absen {obj.name_class}', f'logs\\{datetime.now().strftime("%d %b")}.txt', img_name, mode)
